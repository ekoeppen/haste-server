...
DocumentHandler.prototype.handlePost = function(request, response) {
  ...
  request.on('end', function(end) {
    if (cancelled) return;
    _this.chooseKey(request.headers['title'], function(key) {
...
DocumentHandler.prototype.chooseKey = function(defaultKey, callback) {
  var key;
  if (defaultKey == null || defaultKey.length == 0) {
    key = this.acceptableKey();
  } else {
    key = defaultKey;
  }
  var _this = this;
  this.store.get(key, function(ret) {
    if (ret) {
      _this.chooseKey(defaultKey, callback);
    } else {
      callback(key);
    }
  });
